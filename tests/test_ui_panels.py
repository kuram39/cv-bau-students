...
def test_low_doloznost_warns(monkeypatch):
    """Mostly-weak matched evidence surfaces the 'orientational score' warning —
    driven by evidence tiers (doloženost), not translator confidence."""
    from cv_bau_students.models import SkillFitDetail
    ...
    # Matched skills are all 'weak' (claimed only) → doloznost_label → "nízká".
    candidates_repo.store_match(
        ...
        match=MatchScore(
            ...
            personal_fit=0.0,
            total=40.0,
            confidence_band=10.0,  # confident extraction — proves it's NOT band-driven
            skill_fit_detail=SkillFitDetail(
                role_essential_total=5,
                role_essential_evidenced=2,
                matched_evidence=[("SQL", "weak"), ("Excel", "weak")],
            ),
        ),
    )
...
def test_strong_evidence_no_warning(monkeypatch):
    """Strong matched evidence → no low-doloženost warning even on a wide band."""
    from cv_bau_students.models import SkillFitDetail
    ...
    candidates_repo.store_match(
        ...
        match=MatchScore(
            ...
            personal_fit=0.0,
            total=40.0,
            confidence_band=28.0,  # wide band, but evidence is strong
            skill_fit_detail=SkillFitDetail(
                role_essential_total=5,
                role_essential_evidenced=2,
                matched_evidence=[("SQL", "strong"), ("Python", "strong")],
            ),
        ),
    )
    recruiter_panel._render_detail(ad_id, summaries[0].candidate_id)
    warnings = " ".join(str(c.args[0]) for c in fake.warning.call_args_list if c.args)
    assert "orientační" not in warnings
